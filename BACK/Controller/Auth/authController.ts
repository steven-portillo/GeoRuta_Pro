...
export const datosPerfilSB = async(ctx: Context) => {
  ...
  try {
    ...
    const perfilSB = await Usuario.DatosSesion(sesion.id);
    response.status = 200;
    response.body = perfilSB
  } catch (error) {
    console.log("error auth perfilSB: " + error);
    response.status = 500;
    response.body = { success: false,
